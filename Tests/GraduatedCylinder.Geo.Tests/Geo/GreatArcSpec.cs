...
namespace GraduatedCylinder.Geo
{
    public class GreatArcSpec
    {
        ...
        public void VerifyDistance(double lat1,
                                   double long1,
                                   double lat2,
                                   double long2,
                                   double expectedLengthInMeters) {
            GeoPosition start = new GeoPosition(lat1, long1);
            GeoPosition stop = new GeoPosition(lat2, long2);
            GreatArc.Distance(start, stop).In(LengthUnit.Kilometer).ShouldBeNear(expectedLengthInMeters, .0001);
        }
    }
}