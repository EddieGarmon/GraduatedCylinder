...
public class DensityConversionsFixture
{
    ...
    public void DensityConversions(double value1, MassDensityUnit units1, double value2, MassDensityUnit units2) {
        new MassDensity(value1, units1).In(units2).Value.ShouldBeWithinToleranceOf(value2);
        new MassDensity(value2, units2).In(units1).Value.ShouldBeWithinToleranceOf(value1);
    }
