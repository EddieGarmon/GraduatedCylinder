...
public class FrequencyConversionsFixture
{
    ...
    public void FrequencyConversions(double value1, FrequencyUnit units1, double value2, FrequencyUnit units2) {
        new Frequency(value1, units1).In(units2).Value.ShouldBeWithinToleranceOf(value2);
        new Frequency(value2, units2).In(units1).Value.ShouldBeWithinToleranceOf(value1);
    }
