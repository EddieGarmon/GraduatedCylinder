...
{
    /// <summary>
    ///     Represents the Momentum Dimension
    /// </summary>
    public class Momentum : Dimension, IEquatable<Momentum>, IComparable<Momentum>
    {
        ...
        public static Momentum operator +(Momentum momentum1, Momentum momentum2) {
            ...
            return new Momentum(momentum1.ValueInBaseUnits + momentum2.ValueInBaseUnits) { Units = momentum1.Units };
        }

        public static Momentum operator /(Momentum momentum, double scaler) {
            Guard.NotNull(momentum, "momentum");
            return new Momentum(momentum.ValueInBaseUnits / scaler) { Units = momentum.Units };
        }

        public static Force operator /(Momentum momentum, Time time) {
            Guard.NotNull(momentum, "momentum");
            Guard.NotNull(time, "time");
            double forceValue = momentum.In(MomentumUnit.KilogramMetersPerSecond) / time.In(TimeUnit.Seconds);
        ...
        public static Mass operator /(Momentum momentum, Speed speed) {
            ...
            double massValue = momentum.In(MomentumUnit.KilogramMetersPerSecond) / speed.In(SpeedUnit.MetersPerSecond);
        ...
        public static Speed operator /(Momentum momentum, Mass mass) {
            ...
            double speedValue = momentum.In(MomentumUnit.KilogramMetersPerSecond) / mass.In(MassUnit.Kilograms);
        ...
        public static Energy operator *(Momentum momentum, Speed speed) {
            Guard.NotNull(speed, "speed");
            Guard.NotNull(momentum, "momentum");
            double energyValue = momentum.In(MomentumUnit.KilogramMetersPerSecond) * speed.In(SpeedUnit.MetersPerSecond);
