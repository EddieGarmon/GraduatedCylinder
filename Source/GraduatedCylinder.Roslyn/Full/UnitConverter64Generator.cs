...
[Generator]
public class UnitConverter64Generator : BaseGenerator
{

    public UnitConverter64Generator()
        ...
        foreach (EnumMemberDeclarationSyntax enumMember in @enum.Members) {
            ...
            switch (attribute.AttributeClass?.Name) {
                ...
                case "ScaleInverselyAttribute":
                    ...
                    Buffer.AppendLine(
                        $"\t\t\t\t\tnewValue = {attribute.ConstructorArguments[0].Value} / (baseValue / {attribute.ConstructorArguments[1].Value});");
                    break;
        ...
        foreach (EnumMemberDeclarationSyntax enumMember in @enum.Members) {
            ...
            switch (attribute.AttributeClass?.Name) {
                ...
                case "ScaleInverselyAttribute":
                    //return (_inverseScaleFactor / value) * _scaleFactor;
                    Buffer.AppendLine(
                        $"\t\t\t\t\treturn ({attribute.ConstructorArguments[0].Value} / value) * {attribute.ConstructorArguments[1].Value});");
                    break;
