...
namespace GraduatedCylinder
{
    ...
    ///     respect to time.
    /// </summary>
    public class Speed : Dimension, IEquatable<Speed>, IComparable<Speed>
    {
        ...
        public static Speed operator +(Speed speed1, Speed speed2) {
            ...
            return new Speed(speed1.ValueInBaseUnits + speed2.ValueInBaseUnits) {Units = speed1.Units};
        }

        public static Speed operator /(Speed speed, double scaler) {
            Guard.NotNull(speed, "speed");
            return new Speed(speed.ValueInBaseUnits / scaler) {Units = speed.Units};
        }

        public static Acceleration operator /(Speed speed, Time time) {
            Guard.NotNull(speed, "speed");
            Guard.NotNull(time, "time");
            double accelerationValue = speed.In(SpeedUnit.MetersPerSecond) / time.In(TimeUnit.Seconds);
        ...
        public static Time operator /(Speed speed, Acceleration acceleration) {
            ...
            double timeValue = speed.In(SpeedUnit.MetersPerSecond) / acceleration.In(AccelerationUnit.MetersPerSecondSquared);
        ...
        public static SpeedSquared operator *(Speed left, Speed right) {
            ...
            double value = left.In(SpeedUnit.MetersPerSecond) * right.In(SpeedUnit.MetersPerSecond);
        ...
        public static Energy operator *(Speed speed, Momentum momentum) {
            ...
            double energyValue = speed.In(SpeedUnit.MetersPerSecond) * momentum.In(MomentumUnit.KilogramMetersPerSecond);
        ...
        public static Length operator *(Speed speed, Time time) {
            Guard.NotNull(speed, "speed");
            Guard.NotNull(time, "time");
            double lengthValue = speed.In(SpeedUnit.MetersPerSecond) * time.In(TimeUnit.Seconds);
        ...
        public static Momentum operator *(Speed speed, Mass mass) {
            ...
            double momentumValue = speed.In(SpeedUnit.MetersPerSecond) * mass.In(MassUnit.Kilograms);
        ...
        public static Power operator *(Speed speed, Force force) {
            ...
            Guard.NotNull(speed, "speed");
            double powerValue = speed.In(SpeedUnit.MetersPerSecond) * force.In(ForceUnit.Newtons);
