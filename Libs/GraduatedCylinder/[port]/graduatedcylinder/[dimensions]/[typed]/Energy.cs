...
namespace GraduatedCylinder
{
	...
	/// </summary>
	public class Energy : Dimension, IEquatable<Energy>, IComparable<Energy>
	{
		...
		public static Energy operator +(Energy energy1, Energy energy2) {
			...
			return new Energy(energy1.ValueInBaseUnits + energy2.ValueInBaseUnits) {Units = energy1.Units};
		}

		public static Energy operator /(Energy energy, double scaler) {
			Guard.NotNull(energy, "energy");
			return new Energy(energy.ValueInBaseUnits / scaler) {Units = energy.Units};
		}

		public static Force operator /(Energy energy, Length length) {
			Guard.NotNull(energy, "energy");
			Guard.NotNull(length, "length");
			double forceValue = energy.In(EnergyUnit.NewtonMeters) / length.In(LengthUnit.Meters);
		...
		public static Momentum operator /(Energy energy, Speed speed) {
			...
			Guard.NotNull(energy, "energy");
			double momentumValue = energy.In(EnergyUnit.NewtonMeters) / speed.In(SpeedUnit.MetersPerSecond);
		...
		public static Power operator /(Energy energy, Time time) {
			...
			double powerValue = energy.In(EnergyUnit.NewtonMeters) / time.In(TimeUnit.Seconds);
