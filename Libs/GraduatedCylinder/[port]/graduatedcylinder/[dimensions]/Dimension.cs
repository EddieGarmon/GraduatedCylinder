...
namespace GraduatedCylinder
{
	...
	/// </summary>
	public abstract class Dimension : ISupportUnitOfMeasure
	{
		private const double CloseMargin = 2.2204460492503131E-15;
		...
		/// <returns></returns>
		public double In(UnitOfMeasure desiredUnits) {
		...
		public string ToString(UnitOfMeasure desiredUnits) {
			desiredUnits.DimensionType.ShouldBe(_dimensionType);
			return string.Format("{0} {1}", In(desiredUnits), desiredUnits.Abbreviation);
		}

		public string ToString(UnitOfMeasure desiredUnits, int precision) {
			string format = "{0:N[pre]} {1}".Replace("[pre]", precision.ToString(CultureInfo.InvariantCulture));
		...
		private static bool AreClose(double value1, double value2) {
			...
			return ((-num < difference) && (difference < num));
		}
